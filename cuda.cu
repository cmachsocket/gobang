...
__constant__ int cuda_step_x[MAX_DIRECT + 1]{0, 1, -1, 0, 1};
__constant__ int cuda_step_y[MAX_DIRECT + 1]{0, 1, 1, 1, 0};
...
__device__ __inline__ pair empty_extend(int direct, int _player, int x, int y,int dir) {
    int count = 0, extend_count = 0;
    for (; cuda_is_inside(x, y) and cuda_board[x][y] == _player;
           count++, x += dir*cuda_step_x[direct], y += dir*cuda_step_y[direct]) {

    }
    if (cuda_is_inside(x, y) and cuda_board[x][y] == EMPTY_POS) { //可以继续从空点扩展,前提是扩展后有对应棋子
        for (x += dir*cuda_step_x[direct], y += dir*cuda_step_y[direct];
            cuda_is_inside(x,y) and cuda_board[x][y] == _player;
            extend_count++, x += dir*cuda_step_x[direct], y += dir*cuda_step_y[direct]) {
        }
